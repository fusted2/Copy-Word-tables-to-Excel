...
spinSection = [ [sg.Radio('Equally split tables to sheets', 'RADIO2', default=True, key='equalSplit')],
                [sg.Text('Specify number of tables in each sheet. Ex: 6,7,9 tables in sheet #1, #2 and #3, respectively')],
...
layout = [  [sg.Text('© NGUYEN HOANG PHU, 2021', font=(None, 10, 'bold'), text_color='brown')],
            ...
            [sg.Radio('Existed Excel file:', 'RADIO0', size=(19,1), default=True), sg.InputText(key='excelTemplate'), sg.FileBrowse()],
...
def wordToExcel(dict):
    ...
    sheetNames = wb.sheetnames # creat a list of sheets in the Excel file
    sheets = min(int(dict['sheets']), len(sheetNames)) # in case we enter value > existed sheets


    # handle 'splitSheets'
    if dict['equalSplit'] == False:                                # handle input value
        splitSheets = [int(i) for i in dict['splitSheets'].split(',')]
        if len(splitSheets) > sheets and sheets == 1:
    ...
    for t in range(len(doc.tables)):
        ...
        # set active sheets if we split talbes
        if sheets > 1 and len(splitSheets) > 1:
                ...
                if tableDoc.cell(i, j)._tc not in checkMerge:
                    try:
                        ...
                        try:
                            ...
                            if dict['hyphenToZero'] == True and tableDoc.cell(i, j).text == "-":
                                excelCell.value = 0
                            else:
    ...
    if dict['savedName'].endswith('.xlsx') == False:
        dict['savedName'] += '.xlsx'
